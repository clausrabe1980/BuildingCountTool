---
title: "Open Buildings Temporal V1 — Single ROI (LOCAL)"
output:
  html_document:
    theme: readable
    toc: true
...
These are the libraries used by the app. Together they cover the Shiny runtime, interactive mapping, spatial geometry operations, GeoJSON handling, widget/JS hooks, and plotting.
...
Local directories and file lists for the Temporal V1 layers, V3 layer, and boundary shapefiles.
All these datasets are in the Git Repo and you will need to update file paths to find the data locally.
The year list and FIRST/LAST year constants are used throughout the app for layer selection, map groups, and plotting.
...
# Informational only (already baked into your files)
PRES_THRESH <- 0.5
...
Utility helpers for formatting, ROI preparation, area calculation, filtering, clipping, and safe GeoJSON reading.
These keep the server logic readable and ensure geometry operations are robust (valid polygons, consistent CRS, etc.).
...
## 4) Lazy-load cache for big GeoJSONs

These functions load GeoJSONs on-demand and cache them in memory so the app starts quickly.
Once a year’s layer (or the V3 layer) is loaded the first time, it is reused from RAM for all subsequent ROI clips.

```{r lazy-load-cache}
cache <- new.env(parent = emptyenv())
cache$temporal <- list()  # per-year sf
cache$v3 <- NULL
...
# Local building computations (ROI clip wrappers)
...
## 5) Load Wards and UMN functional areas

These boundary layers support ROI selection by clicking a ward or UMN area.
The code also tries to detect a suitable ID column so polygons can be clicked reliably in Leaflet.
...
The UI sets up the map, controls (Wards / UMN / Draw), histogram controls, time-series plot, and CSV export.
It also includes a small JS message handler so the draw toolbar can be shown/hidden without re-rendering the map.
...
## 7) Map drawing helper functions

These helper functions keep the server code cleaner by grouping common leaflet operations.
They centralise “clear and redraw” logic so filtering and ROI changes don’t duplicate code.
...
draw_polys <- function(poly_sf, group, color, fillOpacity, weight, fillColor = NULL) {
  ...
  if (is.null(poly_sf) || nrow(poly_sf) == 0) return(invisible(NULL))
  leafletProxy("map") %>%
    addPolygons(
      data = poly_sf,
      group = group,
...
draw_wards_all <- function() {
  ...
  if (is.null(wards_sf) || nrow(wards_sf) == 0) return(invisible(NULL))
  leafletProxy("map") %>%
    addPolygons(
      data = wards_sf,
      group = "Wards",
...
draw_umn_all <- function() {
  ...
  if (is.null(umn_sf) || nrow(umn_sf) == 0) return(invisible(NULL))
  leafletProxy("map") %>%
    addPolygons(
      data = umn_sf,
      group = "UMN Areas",
...
## 8) Server state and shared reactives

This sets up shared state (ROI, cached results, toggles) and a status output.
All downstream modules read/write to this shared reactiveValues object so ROI selection and filtering stay consistent.

```{r server-state}
make_server_state <- function(input, output, session) {
...
## 9) Map rendering and draw-control bootstrap

Renders the leaflet map and hides draw controls until “Draw” mode is enabled.
The JS onRender hook removes raw drawn layers so ROI is always rendered consistently via the “ROI” overlay group.

```{r server-map}
register_map <- function(input, output, session, state) {
...
## 10) Size slider UI, histogram, and live map filtering

Creates the min-size input, size-range slider, buildings-in-range text, histogram, and re-draws layers when the range changes.
Filtering happens in-memory on already clipped polygons, so slider changes are fast and do not re-read GeoJSON.

```{r server-size-filtering}
register_size_filtering <- function(input, output, session, state) {
  rv <- state$rv
...
## 11) Time-series computation + plotting

Computes yearly areas (per-year building polygon areas within ROI) and renders the combined bar/line plot.
The plot shows counts as bars and % cover as a line, both filtered by the active building size range.

```{r server-timeseries}
register_timeseries <- function(input, output, session, state) {
  rv <- state$rv
  ...
  list(
    compute_year_area_list = compute_year_area_list,
    ts_filtered = ts_filtered
  )
}
```

## 12) ROI pipeline, mode handlers, exports, and server wiring

Runs the ROI pipeline (clip + cache + plots), handles Wards/UMN/Draw mode events, renders ROI area, and defines the final server().
This is the “orchestration” layer: when ROI changes it recomputes clipped layers and time-series, then updates map + plots.

```{r server-pipeline-and-wiring}
register_pipeline_and_handlers <- function(input, output, session, state, ts_bundle) {
  rv <- state$rv
  status <- state$status
  wards_on <- state$wards_on
  umn_on <- state$umn_on
  draw_on <- state$draw_on
  ...
  run_pipeline_for_roi <- function(roi_sf, label = "ROI") {
    ...
    withProgress(message = "Computing ROI datasets…", value = 0, {
      ...
      incProgress(0.05)
      tryCatch({
        rv$year_area_list <- ts_bundle$compute_year_area_list(roi_sf)
      }, error = function(e) {
        rv$year_area_list <- NULL
...
server <- function(input, output, session) {
  state <- make_server_state(input, output, session)

  register_map(input, output, session, state)
  register_size_filtering(input, output, session, state)

  ts_bundle <- register_timeseries(input, output, session, state)
  register_pipeline_and_handlers(input, output, session, state, ts_bundle)
}
```

## 13) Run the app

Run this chunk interactively (e.g., in RStudio) to launch the app.
It is marked eval=FALSE so knitting the document won’t auto-start a Shiny process during render.
