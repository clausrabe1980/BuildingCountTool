...
#
# FIXED VERSION:
#   1. Package installs now run ONLY for genuinely missing packages, with an
#      explicit CRAN mirror and no install_github() — this was the hang
#   2. Added a startup check reporting any missing data files
#   3. Fixed CSV export crash when no time-series had been computed
#   4. Minor robustness fixes in the ROI clip
# ------------------------------------------------------------


# ------------------------------------------------------------
# PACKAGES — auto-install on first run, no prompts
# ------------------------------------------------------------
# This block is safe to leave in app.R because it ONLY installs packages that
# are genuinely absent. Three rules make it prompt-free:
#   - requireNamespace() checks without attaching, so nothing is ever
#     reinstalled while loaded (that caused the "cannot remove prior
#     installation" errors and the forced session restart)
#   - repos = is set explicitly, so R never opens a CRAN mirror dialog
#   - no install_github(), which asks an unanswerable update question and
#     hangs runApp()
# On a machine that already has everything, this block does nothing.

.required_pkgs <- c("shiny", "leaflet", "leaflet.extras", "sf",
                    "jsonlite", "geojsonsf", "htmlwidgets", "plotly")
...
if (length(.missing_pkgs) > 0) {
  message("--------------------------------------------------------------")
  message("First run on this machine. Installing ", length(.missing_pkgs),
          " missing package(s):")
  message("  ", paste(.missing_pkgs, collapse = ", "))
  message("This can take several minutes. Please wait.")
  message("--------------------------------------------------------------")

  install.packages(.missing_pkgs, repos = "https://cloud.r-project.org")

  # Confirm they actually installed. On Windows, sf and its geospatial
  # dependencies occasionally fail without Rtools; better a clear message
  # than an obscure error 800 lines further down.
  ...
  if (length(.still_missing) > 0) {
    stop(
      ...
      "\nOn Windows, install Rtools first: https://cran.r-project.org/bin/windows/Rtools/",
      "\nThen restart R (Ctrl+Shift+F10) and try again.",
      call. = FALSE
    )
  }
...
TEMPORAL_FILES <- list(
  "2016" = file.path(TEMPORAL_DIR, "V1_2016_UMN_Functional_Areas_3_2026_02_24_10_12_02.geojson"),
  "2017" = file.path(TEMPORAL_DIR, "V1_2017_UMN_Functional_Areas_3_2026_02_24_10_12_04.geojson"),
  "2018" = file.path(TEMPORAL_DIR, "V1_2018_UMN_Functional_Areas_3_2026_02_24_10_12_05.geojson"),
  "2019" = file.path(TEMPORAL_DIR, "V1_2019_UMN_Functional_Areas_3_2026_02_24_10_12_06.geojson"),
  "2020" = file.path(TEMPORAL_DIR, "V1_2020_UMN_Functional_Areas_3_2026_02_24_10_12_07.geojson"),
  "2021" = file.path(TEMPORAL_DIR, "V1_2021_UMN_Functional_Areas_3_2026_02_24_10_12_08.geojson"),
  "2022" = file.path(TEMPORAL_DIR, "V1_2022_UMN_Functional_Areas_3_2026_02_24_10_12_09.geojson"),
  "2023" = file.path(TEMPORAL_DIR, "V1_2023_UMN_Functional_Areas_3_2026_02_24_10_12_11.geojson")
)

V3_FILE <- file.path(V3_DIR, "V3_2023_UMN_Functional_Areas_3_2026_02_24_10_11_57.geojson")
...
UMN_function_areas_path <- "Data/Regions/UMN_Functional_Areas_5.shp"

# ---------------------------
# Startup data check
# ---------------------------
# Paths above are RELATIVE to the app's working directory. If you launch the
# app from somewhere else, or the Data/ folder is missing (e.g. after a fresh
# git clone), everything silently returns empty. This reports it up front.
.expected_files <- c(unlist(TEMPORAL_FILES, use.names = FALSE),
                     V3_FILE, wards_path, UMN_function_areas_path)
.absent <- .expected_files[!file.exists(.expected_files)]

if (length(.absent) > 0) {
  message("--------------------------------------------------------------")
  message("WARNING: ", length(.absent), " of ", length(.expected_files),
          " data files were not found.")
  message("Working directory: ", getwd())
  message("Missing:")
  for (f in .absent) message("  - ", f)
  message("The app will still start, but affected layers will be empty.")
  message("--------------------------------------------------------------")
} else {
  message("All ", length(.expected_files), " data files located.")
}
...
server <- function(input, output, session) {
  ...
  clip_to_roi <- function(sfobj, roi_sf) {
    ...
    # fast prefilter
    idx <- as.vector(sf::st_intersects(sfobj, roi_union, sparse = FALSE))
  ...
  message("Wards loaded: ", if (is.null(wards_sf)) "NULL" else nrow(wards_sf))
  ...
  output$download_csv <- downloadHandler(
    ...
    content = function(file) {
      ...
      ts <- ts_filtered()
      # Fallback must contain every column selected below, or the export errors.
      if (is.null(ts)) {
        ts <- data.frame(
          year                = YEARS,
